const App = (function() {
  return class App {
    ...
    static createNewIdea(event, id) {
      ...
      Adapter.createIdea(id, newIdea).then(data => {
    ...
    static ideaDetails(event, id, userId) {
      ...
      voteContainer.innerHTML = " ";
      chartBox.innerHTML = " ";
      Adapter.getIdeas().then(data => {
        let selectedIdea = data.find(idea => {
          return idea.id === id;
        });
    ...
    static renderResponse(id, feeling) {
      ...
      button.setAttribute(
        "onclick",
        `App.sendResponse(event, ${id}, ${feeling})`
      );
      div.append(h4);
      h4.append(lineBreak);
      div.append(textarea);
    ...
    static updatedDoItCount(event, id, count) {
      ...
      Adapter.updatedDoItCount(id, newCount).then(data => {
        let stats = [];
        let h4 = document.createElement("h4");
            ...
            datasets: [
              {
                ...
                borderColor: ["rgb(192, 192, 192)", "rgb(192, 192, 192)"]
              }
            ]
          },
          options: {
    ...
    static updatedDontDoItCount(event, id, count) {
      ...
      let chart = document.getElementById("pieChart");
      let newCount = count + 1;
      Adapter.updatedDontDoItCount(id, newCount).then(data => {
        let stats = [];
        let h4 = document.createElement("h4");
        h4.innerText = `Thanks for your vote`;
        vote.append(h4);
